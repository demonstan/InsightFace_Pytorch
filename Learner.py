...
class face_learner(object):
    ...
    def save_state(self, conf, accuracy, to_save_folder=False, extra=None, model_only=False):
        ...
        torch.save(
            ...
            ('model_{}_accuracy_{:.4f}_step_{}_{}.pth'.format(get_time(), accuracy, self.step, extra)))
        if not model_only:
            torch.save(
                self.head.state_dict(), save_path /
                ('head_{}_accuracy_{:.4f}_step_{}_{}.pth'.format(get_time(), accuracy, self.step, extra)))
            torch.save(
                self.optimizer.state_dict(), save_path /
                ('optimizer_{}_accuracy_{:.4f}_step_{}_{}.pth'.format(get_time(), accuracy, self.step, extra)))
        ...
        for e in range(epochs):
            ...
            for imgs, labels in pbar:
                ...
                if self.step % self.board_loss_every == 0 and self.step != 0:
                    ...
                    # self.lrscheduler.step(loss_board)
                ...
                if self.step % self.evaluate_every == 0 and self.step != 0:
                    ...
                    print(f"step: {self.step} - loss: {loss_board:.4f} - val_acc: {accuracy:.4f}")
                    self.model.train()
